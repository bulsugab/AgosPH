...
if(isset($_POST["submit2"])){ 
    ...
    if(!empty($_FILES["banner2"]["name"])) { 
        // Get file info 
        $fileName = basename($_FILES["banner2"]["name"]); 
    ...
    if(in_array($fileType, $allowTypes)) { 
            ...
            $image = $_FILES['banner2']['tmp_name']; 
            $imgContent = addslashes(file_get_contents($image)); 
         
        // Insert image content into database 
        $insert = $db->query("UPDATE imgs SET imgFile='$imgContent' WHERE imgClass='banner2'") or die(mysqli_error($db));
