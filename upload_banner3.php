...
if(isset($_POST["submit3"])){ 
    ...
    if(!empty($_FILES["banner3"]["name"])) { 
        // Get file info 
        $fileName = basename($_FILES["banner3"]["name"]); 
    ...
    if(in_array($fileType, $allowTypes)) { 
            ...
            $image = $_FILES['banner3']['tmp_name']; 
            $imgContent = addslashes(file_get_contents($image)); 
         
        // Insert image content into database 
        $insert = $db->query("UPDATE imgs SET imgFile='$imgContent' WHERE imgClass='banner3'") or die(mysqli_error($db));
